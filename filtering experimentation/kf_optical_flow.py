...
class VarianceAnalyzer:
    def __init__(self, keypoint_labels):
        ...
        self.coordinates_history = {label: {'x': [], 'y': []} for label in keypoint_labels}
    ...
    def compute_variances(self, keypoints, labels, variance_threshold=0.0):
        ...
        for i, label in enumerate(labels):
            x_coords = keypoints[:, i, 0]
            y_coords = keypoints[:, i, 1]
...
class PoseEstimator:
    ...
    def _process_frame(self, img, frame_count, pose_history, videoWriter):
        ...
        flow = cv2.calcOpticalFlowFarneback(self.prev_gray, gray_img, None, 0.5, 3, 15, 3, 5, 1.2, 0)

        # Visualize optical flow
        h, w = img.shape[:2]
        y, x = np.mgrid[0:h, 0:w]
        plt.figure(figsize=(10, 10))
        plt.quiver(x, y, flow[..., 0], flow[..., 1], color='r', headlength=1, headaxislength=1, scale=1, scale_units='xy', angles='xy')
        plt.xlim(0, w)
        plt.ylim(h, 0)
