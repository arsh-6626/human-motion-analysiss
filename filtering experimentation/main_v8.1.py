...
class KeypointStabilizer:
    def __init__(self, smoothing_factor=1.0):
        """
        Initialize KeypointStabilizer with exponential smoothing
        
        Args:
            smoothing_factor (float): Controls the weight of new measurements
        """
        self.prev_keypoints = None
        self.smoothing_factor = smoothing_factor

    def stabilize(self, current_keypoints):
        """
        Stabilize keypoints using exponential smoothing
        
        Args:
            current_keypoints (np.array): Current frame keypoints
        
        Returns:
            np.array: Stabilized keypoints
        """
        if self.prev_keypoints is None:
            self.prev_keypoints = current_keypoints
            return current_keypoints

        # Exponential smoothing with adaptive weights
        stabilized_keypoints = (
            self.smoothing_factor * current_keypoints + 
            (1 - self.smoothing_factor) * self.prev_keypoints
        )

        # Update previous keypoints
        self.prev_keypoints = stabilized_keypoints
...
class PoseEstimator:
    def __init__(self, args):
        """
        Initialize pose estimation with given arguments and stabilization
        ...
        self.keypoint_stabilizer = KeypointStabilizer(
            smoothing_factor=args.stabilization_factor
        )
    ...
    def _process_frame(self, img, frame_count, pose_history, videoWriter):
        # Variance plotting and history reset
        if frame_count % self.args.reset_history_frames == 0:
            ...
            plt.ylabel('Covariance')
            plt.grid(True)
            plt.legend(loc='upper right', bbox_to_anchor=(1.25, 1.0), title='Keypoints')
            plt.tight_layout()
            plt.show()
            
            pose_history.clear()

        # Stabilization-focused tracking
        result_person = self.person_model.track(img, persist=True, verbose=False)
        
        if result_person and len(result_person) > 0:
            stable_pose_results = []
            ...
            for r in result_person:
                ...
                # Advanced pose estimation with stability
                ...
                if pose_results and len(pose_results) > 0:
                    ...
                    if np.any(keypoints[:, 2] < self.args.keypoint_threshold):
                        # Skip this frame if any keypoint is below the threshold
                        continue

                    # Temporal smoothing
                    if len(pose_history) > 0:
                        last_keypoints = pose_history[-1]
                        
                        # Weighted averaging for stability
                        smooth_weight = 0.7
                        stabilized_keypoints = (
                            smooth_weight * keypoints + 
                            (1 - smooth_weight) * last_keypoints
                        )
                        
                        # Update keypoints with smoothed version
                        pose_results[0]['keypoints'] = stabilized_keypoints

                    stable_pose_results.extend(pose_results)

            # Process and visualize stabilized poses
            if stable_pose_results:
                # Process pose history with stabilized results
                stable_pose_results = self._process_pose_history(stable_pose_results, pose_history)
                
                vis_img = self._visualize_pose(img, stable_pose_results)
                
                if videoWriter:
                    videoWriter.write(vis_img)
                
                if self.args.show:
                    cv2.imshow('Stabilized Pose Estimation', vis_img)
...
def _visualize_pose(self, img, pose_results):
    ...
    """
    if not pose_results:
        print("Skipping frame: One or more keypoints below threshold.")
        return img
    ...
    return vis_pose_result(
        ...
        pose_results,
        dataset='coco',
        kpt_score_thr=self.args.keypoint_threshold,  # Use the new threshold
...
def parse_arguments():
    ...
    parser.add_argument('--video-path', type=str, 
                        ...
                        help='Video path')
    parser.add_argument('--stabilization-factor', type=float, default=0.7,
                        help='Smoothing factor for keypoint stabilization')
